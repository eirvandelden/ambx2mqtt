...
class AmbxDriverTest < Minitest::Test
  ...
  def test_a_set_is_called_what_the_configuration_calls_it
    driver = driving(controller("serial:AB12CD34"),
                     names: StandInNames.new("serial_AB12CD34" => "Living room"))
  ...
  def test_a_controller_that_is_unplugged_and_comes_back_is_opened_afresh
    ...
    driver = Ambx2mqtt::AmbxDriver.new(controllers, names: StandInNames.new)
    driver.attached_sets
  ...
  def driving(*controllers, names: StandInNames.new)
    Ambx2mqtt::AmbxDriver.new(StandInControllers.new(*controllers), names: names)
  end
end
